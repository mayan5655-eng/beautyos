...
export async function sendWhatsApp(phone, message, options = {}) {
  ...
  // a reminder or a receipt never passes through here. Refused sends are
  // logged as failed with the reason, so they show in her message log and on
  // the dashboard's failure card, and the operator is told once per window.
  if (isPublicType(options.type) && options.tenantId) {
    ...
    if (!verdict.allowed) {
      const refused = {
        ok: false,
        capped: true,
        error: `Daily public-send ceiling reached (${verdict.count}/${verdict.cap})`,
      };
      await logMessage({
        name: options.name,
        phone,
        ...
        type: options.type,
        status: "failed",
        errorDetail: "daily public-send ceiling",
        tenantId: options.tenantId,
      });
      return refused;
    }
