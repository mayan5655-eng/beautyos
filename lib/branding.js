...
export async function fetchPublicSettings(supabase, tenantId) {
  ...
  try {
    const rpc = await supabase.rpc("get_public_branding", { t: tenantId });
    if (!rpc.error && Array.isArray(rpc.data) && rpc.data[0]) return rpc.data[0];
    if (!rpc.error) return null; // RPC live but no such tenant -> no fallback, no cross-tenant
