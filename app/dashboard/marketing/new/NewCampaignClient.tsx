...
export default function NewCampaignClient() {
  ...
  const steps = [
    { num: 1, label: 'פרטי קמפיין', icon: 'pen' as IconName },
    { num: 2, label: 'אסטרטגיה', icon: 'target' as IconName },
    { num: 3, label: 'פוסטים', icon: 'pen' as IconName },
    { num: 4, label: 'קבוצות', icon: 'people' as IconName },
  ]
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold text-gray-800 mb-2"><Icon name="sparkle" size={30}/> יצירת קמפיין חדש</h1>
            ...
            <div key={step.num} className="flex items-center flex-1">
              <div className="flex flex-col items-center flex-1">
                <div className={`w-14 h-14 rounded-full flex items-center justify-center text-2xl font-bold transition-all ${currentStep === step.num ? 'bg-gradient-to-br from-purple-600 to-pink-600 text-white scale-110 shadow-xl ring-4 ring-purple-200' : currentStep > step.num ? 'bg-green-500 text-white shadow-md' : 'bg-gray-200 text-gray-400'}`}>
                  {currentStep > step.num ? '✓' : <Icon name={step.icon} size={22} />}
                </div>
      ...
      {error && (<div className="mb-4 bg-red-50 border-2 border-red-300 text-red-700 p-4 rounded-2xl text-center font-bold shadow-md"><Icon name="warning" size={14}/> {error}</div>)}

      {currentStep === 1 && (
        <div className="bg-white rounded-2xl shadow-lg p-8">
          <h2 className="text-2xl font-bold text-gray-800 mb-2"><Icon name="pen" size={22}/> פרטי הקמפיין</h2>
          ...
          <div className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2"><Icon name="target" size={14}/> מטרת הקמפיין <span className="text-red-500">*</span></label>
            ...
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2"><Icon name="people" size={14}/> קהל יעד</label>
            ...
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-2"><Icon name="spa" size={14}/> סוג הטיפול</label>
      ...
      {currentStep === 2 && (
        <div className="bg-white rounded-2xl shadow-lg p-8">
          <h2 className="text-2xl font-bold text-gray-800 mb-2"><Icon name="target" size={22}/> אסטרטגיית קמפיין</h2>
          <p className="text-gray-500 mb-6">ה-AI ייצור עבורך אסטרטגיה שיווקית</p>
          {!strategy && (<><button onClick={handleGenerateStrategy} disabled={!campaignData.goal || loading || readOnly} title={readOnly ? DISABLED_REASON_HE : undefined} className="w-full py-6 px-8 bg-gradient-to-r from-purple-600 to-pink-600 text-white text-xl font-bold rounded-2xl shadow-xl hover:scale-105 transition disabled:opacity-40 disabled:cursor-not-allowed">{loading ? <Spinner inline label="ה-AI חושב" /> : <><Icon name="sparkle" size={15}/> צרי אסטרטגיה עם AI</>}</button>{!campaignData.goal && <p className="text-center text-amber-600 mt-4 font-semibold">⚠️ חזרי לשלב 1 ומלאי את מטרת הקמפיין</p>}</>)}
          {strategy && (
            <div className="space-y-4 mt-6">
              <div className="bg-purple-50 border-2 border-purple-200 rounded-xl p-5"><h3 className="font-bold text-purple-800 text-lg mb-2"><Icon name="clipboard" size={16}/> הגישה</h3><p className="text-gray-700">{strategy.strategy}</p></div>
              <div className="bg-pink-50 border-2 border-pink-200 rounded-xl p-5"><h3 className="font-bold text-pink-800 text-lg mb-2"><Icon name="palette" size={16}/> טון</h3><p className="text-gray-700">{strategy.tone}</p></div>
              <div className="bg-amber-50 border-2 border-amber-200 rounded-xl p-5"><h3 className="font-bold text-amber-800 text-lg mb-3"><Icon name="bulb" size={16}/> מסרים מרכזיים</h3><ul className="space-y-2">{strategy.keyPoints.map((p, i) => (<li key={i} className="flex gap-2 text-gray-700"><span className="text-amber-600 font-bold">•</span><span>{p}</span></li>))}</ul></div>
              <div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-5"><h3 className="font-bold text-blue-800 text-lg mb-2"><Icon name="people" size={16}/> תובנות קהל</h3><p className="text-gray-700">{strategy.audienceInsights}</p></div>
              <button onClick={() => { setStrategy(null); handleGenerateStrategy() }} className="w-full py-3 bg-gray-100 text-gray-700 font-semibold rounded-xl hover:bg-gray-200"><Icon name="refresh" size={15}/> צרי אסטרטגיה אחרת</button>
            </div>
          )}
        </div>
      )}
            ...
            <div className="space-y-6 mt-6">
              {variations.map((v) => (
                <div key={v.variationNumber} className="bg-gradient-to-br from-purple-50 to-pink-50 border-2 border-purple-200 rounded-2xl p-5 shadow-md">
                  ...
                  <div className="bg-white rounded-xl p-3 mb-3"><p className="text-purple-700 font-semibold mb-1"><Icon name="megaphone" size={14}/> {v.callToAction}</p><p className="text-blue-600 text-sm">{v.hashtags.join(' ')}</p></div>
                  <div className="grid grid-cols-2 gap-3">
                    <button onClick={() => copyPost(v)} className="py-3 bg-purple-600 text-white font-bold rounded-xl hover:bg-purple-700 shadow-md"><Icon name="copy" size={14}/> העתקת טקסט</button>
                    {v.image ? (<button onClick={() => downloadImage(v.image!, v.variationNumber)} className="py-3 bg-pink-600 text-white font-bold rounded-xl hover:bg-pink-700 shadow-md"><Icon name="download" size={14}/> הורדת תמונה</button>) : (<div className="py-3 bg-gray-200 text-gray-500 font-bold rounded-xl text-center">אין תמונה</div>)}
                  </div>
                </div>
              ))}
            </div>
      ...
      {currentStep === 4 && (
        <div className="bg-white rounded-2xl shadow-lg p-8">
          <h2 className="text-2xl font-bold text-gray-800 mb-2"><Icon name="people" size={22}/> קבוצות פייסבוק</h2>
          <p className="text-gray-500 mb-6">ה-AI ימליץ על 10 קבוצות רלוונטיות</p>
          {groups.length === 0 && (<button onClick={handleSuggestGroups} disabled={loading || readOnly} title={readOnly ? DISABLED_REASON_HE : undefined} className="w-full py-6 px-8 bg-gradient-to-r from-purple-600 to-pink-600 text-white text-xl font-bold rounded-2xl shadow-xl hover:scale-105 transition disabled:opacity-40">{loading ? <Spinner inline label="ה-AI מחפש קבוצות" /> : <><Icon name="sparkle" size={15}/> הציעי קבוצות עם AI</>}</button>)}
          {groups.length > 0 && (
              ...
              {groups.map((g, i) => (
                <div key={i} className="bg-gradient-to-r from-blue-50 to-purple-50 border-2 border-blue-200 rounded-xl p-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <button onClick={() => openFacebookSearch(g.name)} className="bg-blue-600 text-white px-4 py-2 rounded-xl font-semibold text-sm hover:bg-blue-700 whitespace-nowrap shadow-md"><Icon name="search" size={14}/> חיפוש</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          {variations.length > 0 && !savedCampaignId && (
            <div className="mt-8 pt-6 border-t-2 border-gray-200">
              <h3 className="text-xl font-bold text-gray-800 mb-3 text-center"><Icon name="check" size={22}/> סיימת!</h3>
              <p className="text-gray-600 mb-4 text-center">שמרי את הקמפיין כדי לחזור אליו בכל זמן</p>
              {saveError && (<div className="mb-4 bg-red-50 border-2 border-red-300 text-red-700 p-3 rounded-xl text-center font-bold"><Icon name="warning" size={14}/> {saveError}</div>)}
              <button onClick={handleSave} disabled={saving || readOnly} title={readOnly ? DISABLED_REASON_HE : undefined} className="w-full py-6 px-8 bg-gradient-to-r from-green-600 to-emerald-600 text-white text-xl font-bold rounded-2xl shadow-xl hover:scale-105 transition disabled:opacity-40">
                {saving ? <Spinner inline label="שומר" /> : <><Icon name="check" size={15}/> שמירת קמפיין</>}
              </button>
            </div>
          )}
          ...
          {savedCampaignId && (
            <div className="mt-8 pt-6 border-t-2 border-gray-200">
              <div className="bg-green-50 border-2 border-green-300 rounded-2xl p-6 text-center">
                <div className="mb-2 text-green-600"><Icon name="check" size={34}/></div>
                ...
                <button onClick={() => router.push('/dashboard/marketing')} className="px-6 py-3 bg-green-600 text-white font-bold rounded-xl hover:bg-green-700 transition shadow-md">
                  <Icon name="clipboard" size={14}/> לרשימת הקמפיינים
                </button>
              </div>
            </div>
          )}
      ...
      <div className="mt-8 flex items-center justify-between gap-4">
        <button onClick={goBack} disabled={currentStep === 1} className="px-8 py-4 bg-white border-2 border-gray-300 text-gray-700 font-bold rounded-2xl shadow-md hover:bg-gray-50 transition disabled:opacity-30 disabled:cursor-not-allowed">← חזרה</button>
