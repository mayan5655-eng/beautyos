        ...
        <main style={{flex:1,overflow:"auto",padding:"22px 18px"}}>
                  ...
                  <div key={client.id} className="client-row" onClick={()=>{setSelectedClient(client);setClientTab("info");}} style={{background:"#fff",borderRadius:16,padding:"12px 14px",border:"1px solid #EFE7EB",display:"flex",alignItems:"center",gap:11,marginBottom:7}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:2,flexWrap:"wrap"}}>
                        ...
                        {days>90&&<span style={{fontSize:7,background:"#FEEBEE",color:"#C62828",padding:"2px 7px",borderRadius:20}}>❄️ {days}י</span>}
          ...
          {activeTab==="whatsapp"&&(()=>{
            ...
            const groups=[
              ...
              {key:"cold",icon:"❄️",title:"מטופלות להתחדשות (60+ יום)",color:"#5580C4",bg:"#EBF3FF",targets:coldTargets,empty:"כל המטופלות פעילות! 🎉"},
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:14}} onClick={()=>setShowClientModal(false)}>
          <div onClick={e=>e.stopPropagation()} className="modal-card" style={{background:"#fff",borderRadius:22,padding:24,width:380,maxWidth:"100%",maxHeight:"90vh",overflowY:"auto"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              ...
              <textarea value={newClient.allergies} onChange={e=>setNewClient({...newClient,allergies:e.target.value})} placeholder="⚠️ אלרגיות" rows={2} style={{width:"100%",border:"1px solid #FFDAC1",borderRadius:12,padding:"9px 12px",fontSize:11,fontFamily:"inherit",outline:"none",direction:"rtl",background:"#FFFAF7",resize:"none"}}/>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:14}} onClick={()=>setShowCashier(false)}>
          <div onClick={e=>e.stopPropagation()} className="modal-card" style={{background:"#fff",borderRadius:22,padding:24,width:420,maxWidth:"100%",maxHeight:"92vh",overflowY:"auto"}}>
            ...
            <div style={{display:"flex",gap:6}}>
              ...
              <button onClick={handleSaveReceipt} disabled={isBusy("saveReceipt")} className="primary-btn" style={{flex:2,padding:"12px 0",background:"linear-gradient(90deg,#C77B92,#D89AAE)",color:"#fff",fontSize:13}}>{isBusy("saveReceipt")?"שומר...":"צרי קבלה ✓"}</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div onClick={e=>e.stopPropagation()} className="client-drawer" style={{background:"#fff",width:440,maxWidth:"100%",height:"100%",overflowY:"auto",boxShadow:"4px 0 30px rgba(0,0,0,0.12)"}}>
            {(()=>{
              ...
              return(<>
                ...
                {(()=>{
                  const insights=[];
                  if(days>90)insights.push({icon:"❄️",text:`לא ביקרה ${days} ימים — שווה הודעת התחדשות`,color:"#5580C4"});
                  if(c.allergies)insights.push({icon:"⚠️",text:`אלרגיות: ${c.allergies}`,color:"#FF9800"});
                  ...
                  if(cPackages.length>0)insights.push({icon:"🎟️",text:`${cPackages.length} חבילות פעילות`,color:"#7B1FA2"});
                ...
                <div style={{padding:"16px 22px"}}>
                  {clientTab==="info"&&(
                    <div style={{display:"flex",flexDirection:"column",gap:9,fontSize:11.5}}>
                      ...
                      {c.allergies&&<div style={{padding:"8px 10px",background:"#FFFAF7",borderRadius:10,border:"1px solid #FFDAC1"}}><p style={{color:"#FF9800",fontWeight:700,fontSize:9,marginBottom:2}}>⚠️ אלרגיות</p><p>{c.allergies}</p></div>}
